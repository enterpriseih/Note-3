...
``` java
package com.example.lib_java.reflect;
...
import com.example.lib_java.reflect.joor.Reflect;
...
public class ReflectDemo {
    public static void main(String[] args) {
        ...
        Person zs = Reflect.on(Person.class).create("zs").get();
        zs.show();
        ...
        Person person = new Person("zs", 18, "nanjing", "20110611");

        Object obj = RefInvoke.createObject("com.example.lib_java.reflect.Person");
        Object address = RefInvoke.getFieldObject(obj, "address");
        System.out.println(address);

        RefInvoke.setFieldObject(obj, "address", "shanghai");
        Object address2 = RefInvoke.getFieldObject(obj, "address");
        System.out.println(address2);

        Object id = RefInvoke.getStaticFieldObject("com.example.lib_java.reflect.Person", "id");
    ...
    private static void print() {
        try {
            Class<TestBean> beanClass = TestBean.class;
            //判断是否是基本数据类型
            boolean primitive = beanClass.isPrimitive();
            Constructor<TestBean> declaredConstructor = beanClass.getDeclaredConstructor();
            if (!declaredConstructor.isAccessible()) {
                declaredConstructor.setAccessible(true);
            }
            TestBean testBean = declaredConstructor.newInstance();
            Method method = beanClass.getDeclaredMethod("printStr");
            method.setAccessible(true);
            int modifiers = method.getModifiers();
            System.out.println("printStr Modifier: " + Modifier.toString(modifiers));
            System.out.println(method.invoke(testBean));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static void test2() {
        try {
            Class<TestBean> beanClass = TestBean.class;
            Constructor<TestBean> declaredConstructor = beanClass.getDeclaredConstructor(String.class);
            declaredConstructor.setAccessible(true);
            TestBean testBean = declaredConstructor.newInstance("staticFinalStr");
            Field strField = beanClass.getDeclaredField("str");
            strField.setAccessible(true);
            strField.set(testBean, "staticFinalStr hello");
    ...
    private static void test1() {
        try {
            ...
            declaredConstructor.setAccessible(true);
            TestBean testBean = declaredConstructor.newInstance("nihao");
            Field strField = beanClass.getDeclaredField("str");
            strField.setAccessible(true);
            System.out.println(testBean.toString());
        } catch (Exception e) {
    ...
    private static void test() {
        try {
            Class<TestBean> beanClass = (Class<TestBean>) 	Class.forName("com.example.lib_java.reflect.TestBean");
            Constructor<TestBean> declaredConstructor = beanClass.getDeclaredConstructor();
            declaredConstructor.setAccessible(true);
            TestBean testBean = declaredConstructor.newInstance();
            Field strField = beanClass.getDeclaredField("staticStr");
            strField.setAccessible(true);
            strField.set(testBean, "static hello");
            System.out.println(testBean.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
